...
\section{Agent Hyper-Parameters and Learning Curves}\label{A:parameters}
	...
	\begin{equation}
		[[64,3,2],~[32,2,1],~[32,2,1],~[512,0.2],~[|\mathcal{A}|, 0]],
	\end{equation}
	with an additional single output for value prediction, as required for dueling methodologies \cite{RLwang2015dueling}, and single convolutional layers have been described with a three-tuple $[n,w,s]$, where $n$ is the number of filters, $w$ is the filter width and $s$ is the stride, and single feed forward layers via a two-tuple $[n,d]$, where $n$ is the number of neurons and $d$ is the output drop-out rate.

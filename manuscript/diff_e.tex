...
%DIF DEL dq_decoding.tex     Mon Oct  1 11:40:03 2018
%DIF ADD dq_decoding_e.tex   Mon Oct  1 17:10:35 2018
...
\section{Introduction}\label{s:introduction}
	...
	that is robust \DIFdelbegin \DIFdel{with respect }\DIFdelend to the unavoidable errors introduced through \DIFdelbegin \DIFdel{the interaction of }\DIFdelend \DIFaddbegin \DIFadd{interaction of the }\DIFaddend physical qubits with a noisy environment.
	A \DIFdelbegin \DIFdel{typical }\DIFdelend \DIFaddbegin \DIFadd{useful }\DIFaddend strategy for achieving such robustness is to encode a single logical qubit into the state of many physical qubits,
	via a quantum error correcting code, from which it may be possible to actively diagnose and correct errors that \DIFdelbegin \DIFdel{might }\DIFdelend \DIFaddbegin \DIFadd{may
	}\DIFaddend occur~\cite{Terhal15,Campbell17}. While many quantum error correcting codes exist, topological quantum
	codes~\cite{Kitaev03, Dennis02, Preskill17lectures, Nayak08, Pachos12, Terhal15, Brown16, Campbell17}\DIFaddbegin \DIFadd{, }\DIFaddend in which only local operations
	...
	\DIFdelbegin \DIFdel{Recently the surface code has emerged as }\DIFdelend \DIFaddbegin \DIFadd{Out of the topological quantum codes, the recently emerged surface code is }\DIFaddend an especially promising \DIFdelbegin \DIFdel{code for large scale fault tolerant quantum computation , }\DIFdelend \DIFaddbegin \DIFadd{candidate for large-scale
	fault-tolerant quantum computation }\DIFaddend due to the combination of its comparatively low overhead and locality requirements, coupled
	with the availability of convenient strategies for the implementation of all required logical gates.

	\DIFdelbegin \DIFdel{Within any such code based }\DIFdelend \DIFaddbegin \DIFadd{In any code-based }\DIFaddend strategy for fault tolerant quantum computation, decoding algorithms play a critical role.
	At a high level \DIFdelbegin \DIFdel{, throughout the course of a computation }\DIFdelend these algorithms take as input the \DIFdelbegin \DIFdel{outcome }\DIFdelend \DIFaddbegin \DIFadd{outcomes }\DIFaddend of diagnostic syndrome measurements \DIFdelbegin \DIFdel{and should }\DIFdelend \DIFaddbegin \DIFadd{(which characterize the
	error that occured at the lower level of the physical qubits), and }\DIFaddend provide as output \DIFdelbegin \DIFdel{suggested }\DIFdelend \DIFaddbegin \DIFadd{a suggestion of
	}\DIFaddend corrections for any errors \DIFdelbegin \DIFdel{which might have occurred, which can then be tracked through the computationand later used to apply corrections to any obtained results.
}\DIFdelend \DIFaddbegin \DIFadd{that might have occured during the computation. These corrections can then be applied either immediately or at
	another desired moment to obtain the correct results.
}

	\DIFaddend It is particularly important to note that in any physically realistic setting the required syndrome measurements are \DIFdelbegin \DIFdel{themselves }\DIFdelend obtained
	via small quantum \DIFdelbegin \DIFdel{circuits}\DIFdelend \DIFaddbegin \DIFadd{circuit measurements too}\DIFaddend , and are therefore \DIFdelbegin \DIFdel{also generically faulty}\DIFdelend \DIFaddbegin \DIFadd{generically also faulty themselves}\DIFaddend .
	...
	decoding algorithm which aims to be experimentally \DIFdelbegin \DIFdel{feasible }\DIFdelend \DIFaddbegin \DIFadd{useful }\DIFaddend should also be capable of dealing with such faulty syndrome measurements.
	...
	enough to not present a bottleneck to the execution of computations, even as the size of the \DIFdelbegin \DIFdel{code scales to larger code distances}\DIFdelend \DIFaddbegin \DIFadd{system (i.e. the code distance) grows}\DIFaddend .
	...
	\DIFaddbegin \DIFadd{and investigated}\DIFaddend , each of which tries to satisfy as many of the experimentally required \DIFdelbegin \DIFdel{criterion }\DIFdelend \DIFaddbegin \DIFadd{criteria }\DIFaddend as possible.
	Perhaps most prominent are algorithms based on minimum-weight perfect matching subroutines~\cite{Fowler13},
	\DIFdelbegin \DIFdel{however }\DIFdelend \DIFaddbegin \DIFadd{although }\DIFaddend alternative approaches based on techniques such as the renormalization group~\cite{Duclos2010} and
	locally operating cellular automata~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Herold15} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Herold15, Kubica2018} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend have also been put forward \DIFdelbegin \DIFdel{.
	}\DIFdelend \DIFaddbegin \DIFadd{as promising candidates.
}

	\DIFaddend Recently, techniques from \DIFdelbegin \DIFdel{classical }\DIFdelend machine learning have begun to find application in diverse areas of quantum physics - such as in the efficient
	representation of many-body quantum states\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Carleo16,todo}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , the identification of phase transitions\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Melko, Liu, todo}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , and
	the autonomous design of novel experimental set-ups\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Bruegel, Coles, todo} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend - and various neural-network based decoders have
	also been proposed~\cite{Torlai10, Varsamopoulos17, Krastanov17, Breuckmann18, Baireuther18a, Baireuther18b, Ni18}.
	\DIFdelbegin \DIFdel{However, despite the }\DIFdelend \DIFaddbegin \DIFadd{Despite this }\DIFaddend diversity of decoding algorithms\DIFdelbegin \DIFdel{now available}\DIFdelend , there is \DIFdelbegin \DIFdel{not }\DIFdelend as of yet \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{no }\DIFaddend algorithm which clearly satisfies all
	the required \DIFdelbegin \DIFdel{criteria, or }\DIFdelend \DIFaddbegin \DIFadd{experimental criteria listed above, nor }\DIFaddend a clear consensus as to which technique would be the most experimentally feasible\DIFdelbegin \DIFdel{in any given experimental context}\DIFdelend .
	\mk{
	...
	In particular, while the \DIFdelbegin \DIFdel{so-far }\DIFdelend \DIFaddbegin \DIFadd{previously }\DIFaddend proposed neural network decoders promise extremely fast decoding times \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and
	}\DIFaddend flexibility with respect to the underlying noise model\DIFdelbegin \DIFdel{and the potential to scale to large code distances}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Ronagh16}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , all such decoders
	are \DIFdelbegin \DIFdel{so far }\DIFdelend restricted either to the setting of perfect syndrome measurements \DIFdelbegin \DIFdel{, }\DIFdelend or to the setting \DIFdelbegin \DIFdel{in which one is trying to store }\DIFdelend \DIFaddbegin \DIFadd{of preserving }\DIFaddend a logical qubit \DIFaddbegin \DIFadd{for }\DIFaddend as long as possible\DIFdelbegin \DIFdel{, without the requirement of performing }\DIFdelend \DIFaddbegin \DIFadd{.
	In the latter scenario there is no requirement on being able to perform }\DIFaddend a subsequent logical gate\DIFdelbegin \DIFdel{.
	As such, while this approach seems promising, there remains }\DIFdelend \DIFaddbegin \DIFadd{, and hence there is }\DIFaddend room for
	improvement \DIFdelbegin \DIFdel{and generalization}\DIFdelend \DIFaddbegin \DIFadd{regarding both scenarios}\DIFaddend .

	Simultaneously, the last few years have also seen \DIFdelbegin \DIFdel{extremely }\DIFdelend impressive advances in the development of deep reinforcement learning algorithms,
	...
	making, involving consideration of the future effects of ones actions. \DIFdelbegin \DIFdel{The problem of decoding within the context of }\DIFdelend \DIFaddbegin \DIFadd{In this work we demonstrate that decoding in the }\DIFaddend fault-tolerant
	\DIFdelbegin \DIFdel{quantum computation seems like exactly such a setting and as such it natural to ask to what extent reinforcement learning techniques could be used to obtain decoding agents, and what advantages such agents might have over existing approaches.
	In this work we set out to provide answers to these questions}\DIFdelend \DIFaddbegin \DIFadd{setting provides a natural setting for the application of reinforcement learning methods. We will discuss the advantages of such
	an approach over the previously proposed network decoders}\DIFaddend .

	In particular, we reformulate the problem of decoding \DIFdelbegin \DIFdel{within the setting of fault-tolerant quantum computation as a process of sequential competitive interaction between a decoding agent and a code environment}\DIFdelend \DIFaddbegin \DIFadd{as an agent performing discrete, sequential actions on the code (which is
	referred to as the environment)}\DIFaddend .
	This reframing \DIFdelbegin \DIFdel{provides a conceptual framework which allows for the application of various deep reinforcement learning algorithms to obtain neural network based decoding agents
	.
	As a proof-of-principle, we then utilize to deepQ learning to obtain fast surface code decoders, }\DIFdelend \DIFaddbegin \DIFadd{of the code as a reinforcement learning environment immediately allows for various different learning agents
	to attempt to learn to decode. In our current approach, we will show that a deepQ-learning agent can decode the surface code }\DIFaddend for
	a variety of noise models \DIFdelbegin \DIFdel{, within }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the fully fault-tolerant setting.
	...
	are then able to provide the conceptual reframing of decoding as a reinforcement learning problem in Section~\ref{s:decoding_as_rl}, \DIFdelbegin \DIFdel{which is }\DIFdelend \DIFaddbegin \DIFadd{representing
	}\DIFaddend one of the primary results of this work.
	In Section~\ref{s:results} we then present deepQ surface code decoders\DIFdelbegin \DIFdel{for a variety of noise models}\DIFdelend , before finally in Section~\ref{s:conclusions} \DIFdelbegin \DIFdel{discussing }\DIFdelend \DIFaddbegin \DIFadd{we
	discuss }\DIFaddend both the advantages and disadvantages of the approach presented here,
...
\section{The Surface Code}\label{s:the_surface_code}
	...
	The framework and methods presented in this work are not restricted to the surface code \DIFdelbegin \DIFdel{, and could }\DIFdelend \DIFaddbegin \DIFadd{however, and may }\DIFaddend be applied to any stabilizer code\DIFdelbegin \DIFdel{, but we choose to restrict ourselves to this code here for both }\DIFdelend \DIFaddbegin \DIFadd{.
	The restriction to the surface code is made both for }\DIFaddend simplicity of presentation and experimental relevance.
	We will focus on presenting the \DIFaddbegin \DIFadd{essential }\DIFaddend elements of the surface code necessary for understanding the decoding problem, \DIFdelbegin \DIFdel{in the process omitting many details, for a more detailed treatment refer to
	refs }\textit{\DIFdel{a,b, and c}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{and refer to
	more complete treatments for details~\mbox{%DIFAUXCMD
\cite{a,b,c}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .

	We will consider $d\times d$ lattices with a \textit{physical\DIFdelbegin \DIFdel{data qubit}\DIFdelend } \DIFaddbegin \DIFadd{data qubit }\DIFaddend on each vertex $v$, as illustrated in Fig.~\ref{f:surface_code} \DIFdelbegin \DIFdel{where }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend $d=5$.
	The collective state of all qubits on the lattice is an element of the Hilbert space $\mathcal{H} = \mathbb{C}^{2^{(d\times d)}}$.
	We associate stabilizer operators with each \DIFdelbegin \DIFdel{coloured }\DIFdelend \DIFaddbegin \DIFadd{colored }\DIFaddend plaquette of the lattice\DIFdelbegin \DIFdel{.
	Stabilizers on }\DIFdelend \DIFaddbegin \DIFadd{, with }\DIFaddend blue (orange) plaquettes \DIFdelbegin \DIFdel{are operators
	which apply }\DIFdelend \DIFaddbegin \DIFadd{representing operators
	applying }\DIFaddend Pauli $X$ ($Z$) flips to \DIFdelbegin \DIFdel{all qubits on }\DIFdelend \DIFaddbegin \DIFadd{the qubits at }\DIFaddend the vertices of the \DIFdelbegin \DIFdel{plaquette}\DIFdelend \DIFaddbegin \DIFadd{plaquettes}\DIFaddend .
	...
	\begin{equation}\label{e:stabilizer_definition}
		...
		\end{cases}
	\end{equation}
	All stabilizers are mutually commuting and have eigenvalues $\pm 1$. \DIFdelbegin \DIFdel{If we define the }\DIFdelend \DIFaddbegin \DIFadd{This allows for the introduction
	of a }\DIFaddend Hamiltonian $H = -\sum_p S_p$, \DIFdelbegin \DIFdel{then }\DIFdelend \DIFaddbegin \DIFadd{from which }\DIFaddend the surface code $\mathcal{H}_\mathrm{sc} \subset \mathcal{H}$ is \DIFdelbegin \DIFdel{the ground state space of this Hamiltonian, which is the space consisting only of simultaneous $+1$ eigenstates of all the stabilizer operators.
	This subspace }\DIFdelend \DIFaddbegin \DIFadd{now defined as
	...
	code }\DIFaddend is two dimensional \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend i.e. $\mathcal{H}_\mathrm{sc} \simeq \mathbb{C}^2$\DIFdelbegin \DIFdel{, and as a result we can }\DIFdelend \DIFaddbegin \DIFadd{) and can hence }\DIFaddend encode a single \textit{logical\DIFdelbegin \DIFdel{qubit}\DIFdelend } \DIFdelbegin \DIFdel{into this code space}\DIFdelend \DIFaddbegin \DIFadd{qubit}\DIFaddend .
	Logical operators are operators which preserve the code space \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{by keeping the state in $\mathcal{H}_\mathrm{sc}$ and thereby
	}\DIFaddend manipulate the state of the logical qubit.
	\DIFdelbegin \DIFdel{As shown in Fig.~\ref{f:surface_code} , }\DIFdelend \DIFaddbegin \DIFadd{Figure~\ref{f:surface_code} shows }\DIFaddend logical $X$ ($Z$) operators, denoted $X_L$ ($Z_L$), \DIFaddbegin \DIFadd{which }\DIFaddend are continuous strings of single
	vertex $X$ ($Z$) operators which connect the top and bottom (left and right) \DIFdelbegin \DIFdel{boundary }\DIFdelend \DIFaddbegin \DIFadd{boundaries }\DIFaddend of the lattice.
	...
	If we assume that the initial state $|\psi\rangle \in \mathcal{H}_\mathrm{sc}$ is an element of the code space, then the \DIFdelbegin \DIFdel{subsequent state
	$|\psi'\rangle \not \in \mathcal{H}_\mathrm{sc}b$ }\DIFdelend \DIFaddbegin \DIFadd{resulting state
	after having incurred a single qubit flip, $|\psi'\rangle \not \in \mathcal{H}_\mathrm{sc}$, }\DIFaddend will no longer be an element of the code space.
	In particular\DIFaddbegin \DIFadd{, }\DIFaddend $|\psi'\rangle$ will be an eigenstate with eigenvalue $-1$ of at least one stabilizer.
	...
	each of which takes the value $\pm 1$. \DIFdelbegin \DIFdel{As such, unlike a truly single qubit encoding of a quantum state, if a
	Pauli flip error occurs on a single physical data qubit}\DIFdelend \DIFaddbegin \DIFadd{The syndrome hence characterizes the physical errors that occured, although not in a
	unique way since multiple error patterns can result in the same syndrome measurement. If only a single error occured however}\DIFaddend ,
	we may be able to \DIFdelbegin \DIFdel{identify and correct this error, in the process conserving }\DIFdelend \DIFaddbegin \DIFadd{correct it and preserve }\DIFaddend the logical qubit\DIFdelbegin \DIFdel{state, by examining the syndrome of the post-error state. This process of }\textit{\DIFdel{decoding}} %DIFAUXCMD
\DIFdel{is discussed }\DIFdelend \DIFaddbegin \DIFadd{'s state. The problem of decoding in the case of multiple errors is
	much harder and is the main topic of this work. We will describe the decoding process }\DIFaddend in the next section.

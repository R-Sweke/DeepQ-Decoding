...
class MySequentialMemory(Memory):
    def __init__(self, limit, **kwargs):
        super(MySequentialMemory, self).__init__(**kwargs)

        self.limit = limit

        # Do not use deque to implement the memory. This data structure may seem convenient but
        # it is way too slow on random access. Instead, we use our own ring buffer implementation.
        self.actions = MyRingBuffer(limit)
        self.rewards = MyRingBuffer(limit)
        self.terminals = MyRingBuffer(limit)
        self.observations = MyRingBuffer(limit)
...
class EpisodeParameterMemory(Memory):
    ...
    def sample(self, batch_size, batch_idxs=None):
        ...
        """
        if batch_idxs is None:
            batch_idxs = sample_batch_indexes(0, self.nb_entries,
                                              size=batch_size)
        assert len(batch_idxs) == batch_size

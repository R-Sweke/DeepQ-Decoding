...
\section{The Decoding Problem}\label{s:the_decoding_problem}
	...
	\end{figure}
	In addition to the inherent difficulty resulting from syndrome ambiguity, in experimental settings the process of extracting the syndrome is also subject to noise, and hence the syndrome itself may be faulty.
	...
	The essential aspect of the additional difficulty from faulty syndrome measurements can be phenomenologically modelled by imagining each time step as consisting of two distinct error processes, as illustrated in Fig.~\ref{f:decoding_problem}.
...
\section{Reinforcement Learning and q-Functions}\label{s:reinforcement_learning}
	...
	Again, we will constrain ourselves to the basics and refer to Refs XXX for a complete treatment.
	...
	This $q$-function is then periodically updated using Eq. \eqref{e:fixed_point}, for which $q_*(s,a)$ is a stationary solution.
	...
	It is precisely to address this problem, and in effect to render $q$-learning applicable in practice, that \textit{deepQ} learning was introduced.
...
\section{Results}\label{s:results}
	...
	For bit-flip (depolarising) noise we find that for approximately $p < 1.4\times 10^{-2}$  $(p < 1\times 10^{-2})$ there is a decoding agent for which the average lifetime of the actively decoded logical qubit is longer than the average lifetime of a single faulty qubit.
	...
	Firstly, both the neural network architecture and reinforcement learning algorithm used here are comparatively simple with respect to the current state-of-the-art, and were chosen to allow for the execution of the required training procedure with the available computational resources.
	...
	Furthermore, as discussed in ref. XXX, using dedicated hardware it is expected that the forward pass time of various applicable neural network architectures (including the architecture used here) can be brought below the time-scales necessary for near-term experiments. 
...
\section{Conclusion}\label{s:conclusions}
	...
	Additionally, the recent use of more sophisticated reinforcement learning techniques and neural network architectures to demonstrate super-human performance in complex domains such as Chess and Go, strongly indicate that the initial results presented here, via deepQ learning with small neural networks, could be successfully improved and extended, given the required computational resources for training.
	...
	Simultaneously, in the perfect-measurements setting, recent effort has focused on developing techniques to allow for the scaling of neural network decoders to larger code distances.
...
\section{Distributed Iterative Training with Simultaneous Hyper-Parameter Optimization}\label{A:training}
	...
	In order to obtain optimal decoders for multiple error rates we implemented a custom iterated training procedure, involving a hyper-parameter grid search at each error rate, as illustrated in Fig. \ref{f:iterative_training}. All code for implementing this procedure on an HPC cluster can be found in the associated \emph{DeepQ-Decoding} repository. In particular, this procedure involves the following steps:
...
\section{Agent Hyperparameters and Learning Curves}\label{A:parameters}
	...
	We implemented the distributed iterative training procedure described in Appendix \ref{A:training}, for both bit-flip and depolarising noise (all utilized code can be found in the \emph{DeepQ-Decoding} repository).
	...
	All agents were trained via the original deepQ algorithm, implemented via the keras-RL library, with annealed $\epsilon$-greedy exploration, doubleQ updates and dueling networks.
	Both fixed and variable hyper-parameter grids are specified in Table. \ref{t:hyper_parameters}.
	Figs. \ref{f:training_results_dp} and \ref{f:training_results_x} show the training history of the optimal decoding agent at each error rate, as well as the associated variable hyper-parameters.

	\begin{table}[h!]
	\begin{center}\caption{Hyper-Parameters}\label{t:hyper_parameters}
	  \begin{tabular}{| c | c | }
	    \hline
	...
	\begin{figure}[ht!]
		\centering
		\includegraphics[width=\linewidth]{figures/depolarising_training.png}
		\caption{Training histories and variable hyper-parameter values for optimal depolarising noise agents. Hyper-parameters are given as a list [Number of exploration steps, initial $\epsilon$, final $\epsilon$, learning rate, target network update frequency].}\label{f:training_results_dp}
	\end{figure}

	\begin{figure}[ht!]
		\centering
		\includegraphics[width=\linewidth]{figures/bitflip_training.png}
		\caption{Training histories and variable hyper-parameter values for optimal bitflip noise agents. Hyper-parameters are listed as per Fig. \ref{f:training_results_dp}.}\label{f:training_results_x}
	\end{figure}

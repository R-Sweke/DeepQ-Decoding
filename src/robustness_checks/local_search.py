...
def sample_syndrome_volume(env, hidden_state):
    ...
    for _ in range(env.d):
        error = env.noise_model.generate_error()
...
num_attacks = 10  # how often we will try an attack
cost_threshold = 2.  # only accept errors that have probability smaller equal than the threshold

visualize = False  # plot surface code in each round
log_msg = True  # print debug messages while attacking
...
for attack in trange(num_attacks):
    ...
    while not episode_done and transitions < max_transitions:
        ...
        if collect_statistics:
            ...
            if transitions < collect_statistics_steps:
                qubit_step_heatmaps[transitions]["all"] += np.sum(
                    np.array(transition_errors) > 0, axis=0)
                qubit_step_heatmaps[transitions]["X"] += np.sum(
                    np.array(transition_errors) == 1, axis=0)
                qubit_step_heatmaps[transitions]["X"] += np.sum(
                    np.array(transition_errors) == 2, axis=0)
                qubit_step_heatmaps[transitions]["Z"] += np.sum(
                    np.array(transition_errors) == 3, axis=0)
                qubit_step_heatmaps[transitions]["Z"] += np.sum(
                    np.array(transition_errors) == 2, axis=0)
        ...
        if visualize:
            ...
            current_true_syndrome = generate_surface_code_syndrome_NoFT_efficient(
                hidden_state, env.qubits)
            draw_surface_code(hidden_state, env.syndromes,
                              current_true_syndrome, env.d)
    ...
    if (transitions >= max_transitions):
        if log_msg:
            print(
                f"Reached maximum number of transitions: {transitions}/{max_transitions}: no attack found!")
        if collect_statistics:
...
if collect_statistics:
    ...
    fig_total.suptitle(
        "Aggregated error distribution over Qubits over multiple steps")
    plt.tight_layout()
    plt.savefig('heatmap_experiments/aggregated_heatmaps.png', dpi=200)

    # Plot step heatmaps
    fig_steps, ax_steps = plt.subplots(
        collect_statistics_steps, 3, sharey=True)
    for i in range(collect_statistics_steps):
